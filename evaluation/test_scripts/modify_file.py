...
class SanityCheckModifier:
    ...
    def create_continuous_predictions_from_gt(self, gt_file: str, variant_name: str,
        ...
        """
        Generate perfect predictions from ground truth with continuous time-series data
        Creates predictions at regular intervals with varying confidence scores for ROC
        """
        if resolution_seconds is None:
            resolution_seconds = 1.0  # Default: predict every second

        gt_lines = self.read_file(gt_file)

        # Extract segments using robust method
        ...
        for seg in segments:
            ...
            # Generate predictions at regular intervals within this segment
            current_time = start_time
            while current_time < end_time:
                # Generate slightly varying confidence scores (still high for perfect)
                # This variation allows ROC curve calculation
            ...
            # Always add final prediction at exact end_time to ensure full coverage
            if current_time != end_time:  # Avoid duplicate if end_time was already included
                confidence = self._generate_confidence_score(None, None, variant_name)
                timestamp_str = end_time.strftime('%Y-%m-%d %H:%M:%S.%f')
                predictions.append(f"{timestamp_str} {sensor} {sensor} ON {activity} {confidence:.6f}")
    ...
    def create_continuous_predictions_with_missing_segments(self, gt_file: str, variant_name: str,
                                                            ...
                                                            activity_filter: List[str] = None) -> List[str]:
        """Generate predictions but randomly skip entire segments"""
        gt_lines = self.read_file(gt_file)
        segments = self._extract_segments_from_lines(gt_lines)

        # Separate filtered and non-filtered segments
        if activity_filter:
            filtered_segments = [s for s in segments if s['activity'] in activity_filter]
            other_segments = [s for s in segments if s['activity'] not in activity_filter]

            # Only apply removal to filtered activities
            num_to_keep = int(len(filtered_segments) * (1 - missing_percent / 100))
            kept_filtered = random.sample(filtered_segments, num_to_keep)
            kept_segments = kept_filtered + other_segments  # Keep all non-filtered
        else:
    ...
    def create_continuous_predictions_with_false_positives(self, gt_file: str, variant_name: str,
        ...
        # Determine which activities to use for false positives
        if activity_filter:
            all_activities = activity_filter
            # Only look for gaps between filtered segments
            filtered_segments = [s for s in segments if s['activity'] in activity_filter]
        else:
        ...
        # Use filtered_segments to find gaps only within target activities
        if len(filtered_segments) > 1:
            # Create list of available gaps
            available_gaps = list(range(len(filtered_segments) - 1))

            # Sample WITHOUT replacement to avoid creating multiple segments at same location
            num_gaps_to_use = min(num_false_segments, len(available_gaps))
            selected_gaps = random.sample(available_gaps, num_gaps_to_use)

            for seg_idx in selected_gaps:
                gap_start = filtered_segments[seg_idx]['end_time'] + timedelta(seconds=1)
                gap_end = filtered_segments[seg_idx + 1]['start_time'] - timedelta(seconds=1)

                if gap_end > gap_start:
                    false_activity = random.choice(all_activities)
                    false_sensor = filtered_segments[seg_idx]['sensor']
    ...
    def create_continuous_predictions_with_label_swap(self, gt_file: str, variant_name: str,
        ...
        # Determine which activities to consider for swapping
        if activity_filter:
            swap_activities = activity_filter
            # Only select segments within filter for swapping
            filterable_indices = [i for i, seg in enumerate(segments) if seg['activity'] in activity_filter]
        else:
            swap_activities = list(set(seg['activity'] for seg in segments))
            filterable_indices = list(range(len(segments)))

        if len(swap_activities) < 2:
            return self.create_continuous_predictions_from_gt(gt_file, variant_name, resolution_seconds)

        # Randomly select segments to swap (only from filterable indices)
        num_to_swap = int(len(filterable_indices) * (swap_percent / 100))
        swappable_indices = random.sample(filterable_indices, num_to_swap)
        segments_to_swap = set(swappable_indices)
        ...
        for idx, seg in enumerate(segments):
            ...
            if is_swapped:
                # Swap activity (only with activities in the filter)
                other_activities = [a for a in swap_activities if a != seg['activity']]
    ...
    def create_sanity_check_variants(self, input_file: str, output_dir: str, activity_filter: List[str] = None):
        """
        ...
        variants = {
            'perfect': {
                ...
                'generator': lambda: self.create_continuous_predictions_from_gt(
                    input_file, 'perfect', resolution_seconds=1.0
                )
            },
            'small_time_shift': {
...
if __name__ == "__main__":
    ...
    variants = modifier.create_sanity_check_variants(input_file, "../results/sanity_checks")
    ...
    print("      '../results/sanity_checks/P001_perfect.txt',")
    print("      '../data/P001.txt'")
    print("  )")
    print("\n  # Plot ROC curves")
