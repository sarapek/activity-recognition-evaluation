...
def read_data():
    ...
    for line in datafile:
        ...
        if len(words) < 4:  # Skip invalid lines
            continue
        date = words[0]
        stime = words[1]
        sensorid = words[2]
        sensorstatus = words[3]
        alabel = words[4] if len(words) > 4 else "Other_Activity"
        ...
        if alabel != "Other_Activity" or not cf.ignore_other:
            end, tempdata = compute_feature(dt, sensorid, sensorid)
...
def compute_feature(dt, sensorid1, sensorid2):
    ...
    if cf.wincnt < (cf.max_window - 1):  # not reached end of window
        ...
        wsize = cf.max_window
        scount = np.zeros(cf.num_sensors, dtype=int)
    ...
    for line in datafile:
        ...
        if first_event == True or timediff.days < 0 or timediff.days > 1:
            for i in range(cf.num_sensors):
                cf.sensortimes[i] = cf.current_timestamp - timedelta(days=1)
            first_event = False
            cf.wincnt = 0  # Reset window count

        if snum1 != -1 and sensorstatus == "ON" and cf.dstype[snum1] == 'n':
            cf.dstype[snum1] = 'm'
